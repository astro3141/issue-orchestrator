...
# gate-command vocabulary in infra/hooks/gate_commands.py. It lives in this
# run's disposable scratch worktree only: the product checkout's rules, the
# repository's shared Codex policy and the operator's ~/.codex are untouched,
# and this file goes away with the worktree. Editing it by hand has no effect
# on the next run.
"""
...
class CodexPlanningCommandGuardInstaller:
    ...
    def establish(
        ...
        probes = self._verify(policy_file)
    ...
    def _verify(self, policy_file: Path) -> tuple[GuardProbe, ...]:
        ...
        """
        probes: list[GuardProbe] = []
        for command, expect_refused in (
            *((sample, True) for sample in PLANNING_REFUSED_SAMPLES),
            *((sample, False) for sample in PLANNING_ALLOWED_SAMPLES),
        ):
            label = " ".join(command)
            try:
                outcome = self._execpolicy.check(policy_file, command)
            except ExecPolicyResultError as exc:
                raise PlanningCommandGuardError(
                    f"The planning command guard at {policy_file} gave no "
            ...
            if refused is not expect_refused:
                ...
                raise PlanningCommandGuardError(
                    f"The planning command guard at {policy_file} does not "
                    f"{wanted} {label!r} (execpolicy answered {outcome.value})"
                )
            probes.append(GuardProbe(command=tuple(command), refused=refused))
        return tuple(probes)
